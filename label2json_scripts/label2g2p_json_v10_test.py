  ...
  for i in range(0, len(trans_lists), 2):
    ...
    for question, answer in zip(question_list, answer_list):
      ...
      sub_tmp_dict1 = {"from": "system", 
                      "value": "对提供的中文文本根据下面规则进行处理，来完成韵律标记和多音字选择的任务，文字标点符号保持不变，只选择多音字括号内的拼音。韵律标记：为句子中的每个字添加韵律标签，只有3类标签'#1,#2,#3'。规则如下：词：成词的两个或多个字的最后一个字后面标记为#1；短语：短语中的最后一个字后面标记为#2；如果字后面跟随标点符号（且该字不是句子的最后一个字），或者单句无标点符号文本并且文本过长，需要根据语意进行切分停顿，则该字的标记为#3。多音字选择：对用户提供的多音字，根据上下文选择一个最合适的拼音。"}
...
def text2json(text_path, polyphone_dict, output_dir=''):
  ...
  """
  trans_lists = []
  train_output_file = os.path.join(output_dir, "g2p_train_prompt_v10_test.json")
  dev_output_file = os.path.join(output_dir, "g2p_dev_prompt_v10_test.json")
  truth_dev_file = os.path.join(output_dir, "truth_dev_v10_test.txt")
