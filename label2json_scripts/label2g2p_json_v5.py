...
def gen_json_data(trans_lists, polyphone_dict):
  data = []
  ...
  for i in range(0, len(trans_lists), 2):
    ...
    sub_tmp_dict1 = {"from": "system", 
                     "value": "对提供的中文文本根据下面规则进行处理，来完成韵律标记和多音字选择的任务。韵律标记：为句子中的每个字添加韵律标签，只有3类标签'#1,#2,#3'。规则如下：词：成词的两个或多个字的最后一个字后面标记为#1；短语：短语中的最后一个字后面标记为#2；如果字后面跟随标点符号（且该字不是句子的最后一个字），或者单句无标点符号文本并且文本过长，需要根据语意进行切分停顿，则该字的标记为#3。多音字选择：对用户提供的多音字，根据上下文选择一个最合适的拼音，例如这个例子，问：“很重[zhong4|chong2]要”，答：“很#1重[zhong4]要”。"}
...
def text2json(text_path, polyphone_dict, output_dir=''):
  """ rename id
    Args:
  ...
  """
  trans_lists = []
  train_output_file = os.path.join(output_dir, "g2p_train_prompt_v6_data1.json")
  dev_output_file = os.path.join(output_dir, "g2p_dev_prompt_v6_data1.json")
  truth_dev_v5_file = os.path.join(output_dir, "truth_dev_v6_data1.txt")
  ...
  train_data, _ = gen_json_data(trans_lists[:19400] + trans_lists[20000:], polyphone_dict)
  dev_data, truth_answer_dev_data = gen_json_data(trans_lists[19400:20000], polyphone_dict)

  with open(truth_dev_v5_file, 'w') as f:
...
def __cmd():
  ...
  parser.add_argument("--polyphone_file",type=str,
    default='/mnt/cfs/SPEECH/liuhuang1/workspace/tts_frontend/rm_common_polyphone.txt',
    required=False)
  parser.add_argument(
      "--output_dir",
      type=str,
  ...
  text2json(text_file, polyphone_dict, output_dir)
