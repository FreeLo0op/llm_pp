...
merged_model_path = "/mnt/cfs/SPEECH/liuhuang1/workspace/llm_pp/merged_v13_ckt3780"
...
def label2text(text_path, polyphone_dict, en_polyphone_dict, test_dev=False):
  ...
  data = []
  for i in range(0, len(trans_lists), 1):
    sent = trans_lists[i]
    print(sent)
    ...
    for j in range(len(sent_list)):
      ...
      if is_english_for_spss(tmp_text) and tmp_text in en_polyphone_dict_keys:
        # 因为字节的重音或者音素可能不一致，需要把字节的加入到多音字选择列表

        # add_zijie = False
        polyphone_tmp = en_polyphone_dict[tmp_text]
        polyphone_tmp = re.sub(r'[\[,\]]', r'', polyphone_tmp)
        polyphone_tmp = polyphone_tmp.split('|')
        # current_phonemes = ' '.join(item[1])

        # print(polyphone_tmp)
        # print(current_phonemes, '\n')

        # 字节的音素和字典音素不一致，需要把字节的加入到多音字选择列表
        # for polyphone_item in polyphone_tmp:
        #   if len(polyphone_item) == len(current_phonemes):
        #     add_zijie = False
        #     for item1,item2 in zip(polyphone_item, current_phonemes):
        #       if item1 != item2:
        #         add_zijie = True
        #         break
        #     # 找到了一摸一样的音素
        #     if add_zijie is False:
        #       break

        # if add_zijie:
        #   question += '[' + current_phonemes + '|' + '|'.join(polyphone_tmp) + ']'
        # else:
        question += en_polyphone_dict[tmp_text]

      # add english space
      if is_english_for_spss(tmp_text) and j+1 < len(sent_list) and is_english_for_spss(sent_list[j+1]):
        question += ' '
...
def __cmd():
  ...
  parser.add_argument(
      "--filename",
      type=str,
      default='infer_pp_7b_prompt_v13_ckt3780_xiaosi_emotions.txt',
      required=False,
  ...
  with open(output_file, 'w', encoding='utf-8')as fw:
    ...
    for item in tqdm(data_inf):
      ...
      time1 = time.time()
      print(item[1])
      result = llm_pp_inf(item[1])
      text_prosody = result.strip("Answer: ")
      # print(f"result: {result}")
      time2 = time.time()
      consume_time = time2 - time1
      print(f"time: {consume_time} {consume_time/len(item[1])}")
      # write data
      fw.writelines(sid+ "\t" + text_prosody + '#4。' + '\n')
      # exit()
      if i % 10 == 0:
        fw.flush()
      i += 1
