...
lora_path = '/mnt/cfs/SPEECH/liuhuang1/workspace/llm_pp/pp_qwen1half_7b_chat_prompt_v6_data1/qwen1half-7b-chat/v0-20240522-130951/checkpoint-1350/'
...
对提供的中文文本根据下面规则进行处理，来完成韵律标记和多音字选择的任务。韵律标记：为句子中的每个字添加韵律标签，只有3类标签'#1,#2,#3'。规则如下：词：成词的两个或多个字的最后一个字后面标记为#1；短语：短语中的最后一个字后面标记为#2；如果字后面跟随标点符号（且该字不是句子的最后一个字），或者单句无标点符号文本并且文本过长，需要根据语意进行切分停顿，则该字的标记为#3。多音字选择：对用户提供的多音字，根据上下文选择一个最合适的拼音，例如这个例子，问：“很重[zhong4|chong2]要”，答：“很#1重[zhong4]要”。
<|im_end|>
<|im_start|>user
Question: """
...
def __cmd():
  ...
  output_file = os.path.join(output_dir, "infer_pp_7b_prompt_v6_data1_ckt340.txt")
