...
merged_model_path = "/mnt/cfs/SPEECH/liuhuang1/workspace/llm_pp/merged_v17_4b_ckt8620"
...
对提供的中文文本根据下面规则进行处理，来完成韵律标记和多音字选择的任务，文字标点符号和顺序保持不变。韵律标记：为句子中的每个字添加韵律标签，只有3类标签'%,$,&'。规则如下：词：成词的两个或多个字的最后一个字后面标记为'%'；短语：短语中的最后一个字后面标记为'$'；如果字后面跟随标点符号（且该字不是句子的最后一个字），或者单句无标点符号文本并且文本过长，需要根据语意进行切分停顿，则该字的标记为'&'。多音字选择：用户提供的多音字在括号'[]'中，且用'|'分割'[]'里的各个多音字数字类型标识，根据上下文选择一个括号内最合适的数字拼音标识。
<|im_end|>
<|im_start|>user
"""
...
def recover_prosody_pinyin(question, answer, polyphone_dict, en_polyphone_dict):
  ...
  # 括%[1] -> 括[1]%
  sent_cont = re.sub(r'([%$&])(\[[1-9]\])', r'\2\1', answer)
  ...
  # merge [  1  ] -> [1]
  sent_cont = re.sub(r'\[\s+(\d)\s+\]', r'[\1]', sent_cont) 
  sent_cont = re.sub(r'\'', r'', sent_cont)
  # add the blanks for words (whether is chinese characters)
  ...
  for i in range(len(sent_list)):
    ...
    if i+1 < len(sent_list):
      next_prosody = sent_list[i+1]

    if prosody in "%$&":
      results += convert_symbol2prosody_level(prosody)
    elif prosody.startswith('[') and prosody.endswith(']'):
      continue
    elif is_mandarin_for_spss(prosody) or is_english_for_spss(prosody):
      results += prosody
      if next_prosody.startswith('[') and next_prosody.endswith(']'):
        poly_index = int(next_prosody[1:-1]) - 1
        if prosody in polyphone_dict.keys():
          results += '[' + polyphone_dict[prosody][1][poly_index] + ']'
        elif prosody in en_polyphone_dict.keys():
          results += '[' + en_polyphone_dict[prosody][1][poly_index] + ']'
    else:
       results += prosody
    if is_english_for_spss(prosody) and is_english_for_spss(next_prosody):
      results += ' '
  ...
  for i in range(0, len(trans_lists), 2):
    ...
    for j in range(len(align_lists)):
      ...
      if is_mandarin_for_spss(tmp_text) and tmp_text in polyphone_dict_keys:
        question += polyphone_dict[tmp_text][2]
...
def __cmd():
  ...
  parser.add_argument(
      "--filename",
      type=str,
      default='infer_pp_7b_prompt_v17_ckt8620_all.txt',
      required=False,
  ...
  with open(output_file, 'w', encoding='utf-8')as fw, open(output_file1, 'w', encoding='utf-8')as fw1:
    ...
    for item in tqdm(data_inf):
      ...
      try:
        results = recover_prosody_pinyin(item[3], text_prosody, polyphone_dict, en_polyphone_dict)
      except Exception as e:
        print(e)
        print(sid, '\t', item[3])
        print(text_prosody)
