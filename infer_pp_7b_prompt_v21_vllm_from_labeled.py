...
merged_model_path = "/mnt/cfs/SPEECH/liuhuang1/workspace/llm_pp/merged_v19_4b_ckt1330"
...
对提供的中文文本根据下面规则进行处理，来完成韵律标记和多音字选择的任务，文字标点符号和顺序保持不变。韵律标记：为句子中的每个字添加韵律标签，只有3类标签'#1,#2,#3'。规则如下：词：成词的两个或多个字的最后一个字后面标记为#1；短语：短语中的最后一个字后面标记为#2；如果字后面跟随标点符号（且该字不是句子的最后一个字），或者单句无标点符号文本并且文本过长，需要根据语意进行切分停顿，则该字的标记为#3。多音字选择：用户提供的多音字在括号'[]'中，且用'|'分割'[]'里的各个多音字，根据上下文选择一个括号内最合适的拼音。
<|im_end|>
<|im_start|>user
"""
...
def label2text(text_path, polyphone_dict, en_polyphone_dict, test_dev=False):
  ...
  for i in range(0, len(trans_lists), 2):
    ...
    for j in range(len(align_lists)):
      ...
      # if is chinese polyphone, not erhua
      if is_mandarin_for_spss(tmp_text) and tmp_text in polyphone_dict_keys:
        question += tmp_text
        answer += tmp_text
        question += polyphone_dict[tmp_text][0]
      ...
      # if is erhuayin, only erhua text is two characters
      elif num_mandarin(tmp_text) > 1:
        # huar4 -> hua4
        erhua_pinyin = item[1][:-2] + item[1][-1]
        erhua_first = tmp_text[0]
        erhua_second = tmp_text[1]

        question += erhua_first
        answer += erhua_first
        if is_mandarin_for_spss(erhua_first) and erhua_first in polyphone_dict_keys:
          question += polyphone_dict[erhua_first][0]
          ## in the polyphone_dict, add it
          if erhua_pinyin in polyphone_dict[erhua_first][1]:
            answer += '[' + erhua_pinyin + ']'
          # not in the polyphone_dict, add the first
          else:
            answer += '[' + polyphone_dict[erhua_first][1][0] + ']'

        question += erhua_second
        answer += erhua_second
        if is_mandarin_for_spss(erhua_second) and erhua_second in polyphone_dict_keys:
          question += polyphone_dict[erhua_second][0]
          answer += '[' + 'xer2' + ']'
         
      elif is_english_for_spss(tmp_text) and tmp_text in en_polyphone_dict_keys:
        english_poly_nums += 1
        question += tmp_text
        answer += tmp_text
    ...
    data.append([sid, trans_lists[i+1], question])
  return data
...
def __cmd():
  ...
  parser.add_argument("--polyphone_file",type=str,
    default='/mnt/cfs/SPEECH/liuhuang1/workspace/tts_frontend/merge_polyphone_er.txt',
    required=False)
  ...
  parser.add_argument(
      "--filename",
      type=str,
      default='infer_pp_7b_prompt_v19_ckt1330_labeled_all.txt',
      required=False,
  ...
  with open(output_file, 'w', encoding='utf-8')as fw:
    ...
    for item in tqdm(data_inf):
      ...
      time1 = time.time()
      print(item[2])
      result = llm_pp_inf(item[2])
      text_prosody = result.strip("Answer: ")
      # print(f"result: {result}")
      time2 = time.time()
      consume_time = time2 - time1
      print(f"time: {consume_time} {consume_time/len(item[2])}")
      # write data
